// Package logo renders a LOGS wordmark in a stylized way.
package logo
...
// Render renders the LOGS logo. Set the argument to true to render the narrow
...
// SmallRender renders a smaller version of the LOGS logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(width int) string {
	t := styles.CurrentTheme()
	title := t.S().Base.Foreground(t.Secondary).Render("Charm™")
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad("LOGS", t.Secondary, t.Primary))
